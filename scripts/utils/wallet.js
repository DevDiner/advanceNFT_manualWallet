...
class ManualWallet {
    ...
    async getNonce() {
        const nonceHex = await this.rpc("eth_getTransactionCount", [this.address, "pending"]);
        return BigInt(nonceHex);
    }

    async estimateGas(to, value, data) {
        const gasHex = await this.rpc("eth_estimateGas", [{ from: this.address, to, value, data }]);
    ...
    async buildAndSendTx(to, valueWei = 0n, data = "0x") {
        ...
        const gasLimit = await this.estimateGas(to, "0x" + valueWei.toString(16), data);
        ...
        const unsignedTx = [
            Buffer.from(this.chainId.toString(16).padStart(2, '0'), 'hex'),
        ...
        const txPayload = Buffer.concat([Buffer.from('02', 'hex'), rlp.encode(unsignedTx)]);
