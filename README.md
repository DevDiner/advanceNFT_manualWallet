...
2.  **Terminal 2: Deploy Smart Contracts**
    ...
    npm run deploy
    ```
    **Action:** This creates `deployed-addresses.json`, `public/merkle-proofs.json`, `relayer-artifacts.json`, and the `.env.local` file for the frontend.
...
2.  **Run the Sepolia Deployment Script:**
    ...
    *   Generates `deployed-addresses.json` and `relayer-artifacts.json` for the backend relayer.
...
1.  **Add the files to Git:**
    ```bash
    git add public/merkle-proofs.json deployed-addresses.json relayer-artifacts.json
    ```
2.  **Commit and Push:**
    ```bash
